...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		while (true)
		{
			try
			{
				//Thinking for random time
				sleep(random.nextInt(10000));
				//Become hungry after thinking
				table.Hungry_GUI(id);
				take_forks(id);
				table.Eating_GUI(id);
				put_forks(id);
	...
	public void take_forks(int id) throws InterruptedException
	{
		try
		{
			mutex.acquire();
	...
	public void put_forks(int id) throws InterruptedException {
		int LEFT = (id + N - 1) % N;
		int RIGHT = (id + 1) % N;
		try
		{
